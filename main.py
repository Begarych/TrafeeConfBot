...
async def start(message: telebot.types.Message):
    ...
    await bot.send_message(message.chat.id, text=f'Привет, {message.chat.first_name}! 🙋‍♂\n\n️Это Trafee Conf Bot, который '
...
async def instruction(message: telebot.types.Message):
    await bot.send_message(message.chat.id, text="📨 Введите ваш адрес электронной почты\n\n Пример: example@example.com",
...
async def confirmed(message: telebot.types.Message):
    ...
    await bot.send_message(message.chat.id, text=f"📩 Ваш адрес электронной почты успешно сохранен.",
                           reply_markup=ReplyKeyboardRemove())
    await bot.send_message(message.chat.id, text=f"Чтобы завершить регистрацию, пожалуйста, перейдите по ссылке",
                           reply_markup=keyboard)
    sleep(10)
    await bot.send_message(message.chat.id, text="Не забудьте проверить и подтвердить свой электронный адрес. Также, "
...
async def already_register(message: telebot.types.Message):
    await bot.send_message(message.chat.id, text="Вы уже были зарегистрированы\n"
...
async def useless(message: telebot.types.Message):
    await bot.send_message(message.chat.id, text="Используйте /start для старта бота")
